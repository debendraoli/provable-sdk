use std::os::raw::c_char;

use rand::thread_rng;
use snarkvm_console::{
    account::{Address, ComputeKey, GraphKey, PrivateKey, ViewKey},
    network::MainnetV0,
};

use crate::helpers::{ffi_catch, read_c_str, ParseAleo};

type N = MainnetV0;

/// Generate a new random Aleo private key.
/// Returns the key as a string, e.g. "APrivateKey1zkp...".
/// On error returns `{"error":"..."}`.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_private_key_new() -> *mut c_char {
    ffi_catch!("panic during key generation",
        let rng = &mut thread_rng();
        let sk = PrivateKey::<N>::new(rng).map_err(|e| e.to_string())?;
        Ok(sk.to_string())
    )
}

/// Derive the view key from a private key string.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_private_key_to_view_key(sk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during view key derivation",
        let sk_str = unsafe { read_c_str(sk_ptr) }.ok_or("null private key pointer")?;
        let sk = PrivateKey::<N>::parse_aleo(sk_str)?;
        let vk = ViewKey::try_from(&sk).map_err(|e| e.to_string())?;
        Ok(vk.to_string())
    )
}

/// Derive the address from a private key string.
/// Deprecated: Use `aleo_derive_all_keys` instead.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_private_key_to_address(sk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during address derivation",
        let sk_str = unsafe { read_c_str(sk_ptr) }.ok_or("null private key pointer")?;
        let sk = PrivateKey::<N>::parse_aleo(sk_str)?;
        let addr = Address::try_from(&sk).map_err(|e| e.to_string())?;
        Ok(addr.to_string())
    )
}

/// Derive the address from a view key string.
/// Deprecated: Use `aleo_derive_all_keys` instead.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_view_key_to_address(vk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during address derivation from view key",
        let vk_str = unsafe { read_c_str(vk_ptr) }.ok_or("null view key pointer")?;
        let vk = ViewKey::<N>::parse_aleo(vk_str)?;
        let addr = Address::try_from(&vk).map_err(|e| e.to_string())?;
        Ok(addr.to_string())
    )
}

/// Derive the compute key from a private key string.
/// Deprecated: Use `aleo_derive_all_keys` instead.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_private_key_to_compute_key(sk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during compute key derivation",
        let sk_str = unsafe { read_c_str(sk_ptr) }.ok_or("null private key pointer")?;
        let sk = PrivateKey::<N>::parse_aleo(sk_str)?;
        let ck = ComputeKey::try_from(&sk).map_err(|e| e.to_string())?;
        Ok(serde_json::to_string(&ck).map_err(|e| format!("serialize compute key: {e}"))?)
    )
}

/// Derive the graph key from a view key string.
/// Deprecated: Use `aleo_derive_all_keys` instead.
#[unsafe(no_mangle)]
pub extern "C" fn aleo_view_key_to_graph_key(vk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during graph key derivation",
        let vk_str = unsafe { read_c_str(vk_ptr) }.ok_or("null view key pointer")?;
        let vk = ViewKey::<N>::parse_aleo(vk_str)?;
        let gk = GraphKey::try_from(&vk).map_err(|e| e.to_string())?;
        Ok(gk.to_string())
    )
}

/// Derive all keys from a private key in a single FFI call.
/// Returns a JSON object: {"view_key":"...","address":"...","compute_key":"...","graph_key":"..."}
#[unsafe(no_mangle)]
pub extern "C" fn aleo_derive_all_keys(sk_ptr: *const c_char) -> *mut c_char {
    ffi_catch!("panic during key derivation",
        let sk_str = unsafe { read_c_str(sk_ptr) }.ok_or("null private key pointer")?;
        let sk = PrivateKey::<N>::parse_aleo(sk_str)?;
        let vk = ViewKey::try_from(&sk).map_err(|e| e.to_string())?;
        let addr = Address::try_from(&sk).map_err(|e| e.to_string())?;
        let ck = ComputeKey::try_from(&sk).map_err(|e| e.to_string())?;
        let gk = GraphKey::try_from(&vk).map_err(|e| e.to_string())?;
        let result = serde_json::json!({
            "view_key": vk.to_string(),
            "address": addr.to_string(),
            "compute_key": ck,
            "graph_key": gk.to_string(),
        });
        Ok(result.to_string())
    )
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::helpers::test_helpers::{c, call_ffi};

    #[test]
    fn test_keygen_roundtrip() {
        let sk_str = call_ffi(|| aleo_private_key_new());
        assert!(sk_str.starts_with("APrivateKey1"));

        let csk = c(&sk_str);
        let vk_str = call_ffi(|| aleo_private_key_to_view_key(csk.as_ptr()));
        assert!(vk_str.starts_with("AViewKey1"));

        let addr_str = call_ffi(|| aleo_private_key_to_address(csk.as_ptr()));
        assert!(addr_str.starts_with("aleo1"));

        let cvk = c(&vk_str);
        let addr2_str = call_ffi(|| aleo_view_key_to_address(cvk.as_ptr()));
        assert_eq!(addr_str, addr2_str);
    }

    #[test]
    fn test_null_private_key() {
        let s = call_ffi(|| aleo_private_key_to_view_key(std::ptr::null()));
        assert!(s.contains("error"));
    }
}
